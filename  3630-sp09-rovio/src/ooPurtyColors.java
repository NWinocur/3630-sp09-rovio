...
public class ooPurtyColors extends Planner {
	...
	/**
	 * copypasta of old, ugly makeMove code more like what was used in previous
	 * demo. For reference only, probably not a good idea to call it.
	 */
	private void ancientMakeMove() {
		...
		while (finished == false) {
			...
			if (null == hueSegmented) {
				...
				System.out
				.println("Are all corner coordinates in view? "
				...
				double distanceFromMarker = distanceTable
				.getDistance(avgHeight);
				System.out.println("Distance from marker: "
						+ distanceFromMarker);
	...
	public void makeMove() {
		...
		// targetingData: array of [r, y, g, b, v][target, window, minsat]
		ColorSpace ooPurtyColorsCSpace = new ColorSpace();
		int[][] targetingData = ooPurtyColorsCSpace.getTargetingData();
		ImageProc oopcIP = new ImageProc();
		int confusedTurns = 0;
		...
		while (finished == false) {
			...
			BufferedImage noiseReduced = oopcIP.reduceNoise(oopcIP
			...
			if (looksLikeWeAreParked(hueSegmented, targetHue,
					targetHueWindow, minSatToBeUseful)) {
				super.robot.lookMid();
				BufferedImage parkCheckImageArray[] = burstFire(burstLength);
				BufferedImage noiseReducedParkCheck = reduceNoise(parkCheckImageArray);
