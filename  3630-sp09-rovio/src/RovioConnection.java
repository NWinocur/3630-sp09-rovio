...
public class RovioConnection {
	...
	public InputStream open(String resourceName) throws IOException {
		try {
			return open(new URL("http", host, resourceName));
	...
	public static void main(String[] args) {
		...
		RovioConnection rovio = new RovioConnection("http://3630rovio.servebeer.com", "admin", "3630class");
		
		try {
			// Query the robot's status
			URL url = new URL(rovio.getHost() + "/rev.cgi?Cmd=nav&action=1");
			InputStream in = rovio.open(url);
